...
> ⚠️ **Status — mid-migration to core; code ≠ runtime.** This plugin began as a standalone plugin and is being reworked into a bundled core plugin. The sections below document what the **code on disk implements**, but several of those features are **not currently working** in the integrated core build and need re-wiring against the current host APIs — known not-working: folder management (create/rename/delete), nested subfolders, collapsible folders + expand/collapse-all, live search, and the `/` keyboard shortcut. Hover metadata badges have been dropped. Treat the detailed sections as a map of the existing implementation to repair, not a list of confirmed-live features. Verify against a running build before relying on any of it.
